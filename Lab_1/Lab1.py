...
end = False  # boolean for if the game is ended

# ----------------------------------------------------------------------------------------------------------------------
# Paddles functions
# ----------------------------------------------------------------------------------------------------------------------
# Purpose: draw left paddle
# Parameters: x and y coordinates of the top left-hand corner of the paddle;
# w and h are the width and height of the paddle, respectively
def l_paddle(x, y, w, h):
    set_fill_color(0.5, 0.8, 0.2)
    disable_stroke()
    draw_rectangle(x, y, w, h)

# Purpose: draw right paddle
# Parameters: x and y coordinates of the bottom righ-hand corner of the paddle;
# w and h are the width and height of the paddle, respectively
...
# ----------------------------------------------------------------------------------------------------------------------
# Purpose: move left paddle
# Parameter: number of pixels to move the paddle
def l_paddle_move(m):
    global lx, ly
    ly += m
    l_paddle(lx, ly, PADDLE_WIDTH, PADDLE_HEIGHT)

# Purpose: move right paddle
# Parameter: number of pixels to move the paddle
def r_paddle_move(m):
    global rx, ry
    ry -= m
...
# Purpose: determines how various keys interact with the game when pressed
# Parameter: keystroke input
def key(val):
...
# Purpose: determines how various key interact with the game when released
# Parameter: keystroke input
def release(val):
...
# Ball functions
# ----------------------------------------------------------------------------------------------------------------------
# Purpose: draw the ball
# Parameter: x and y coordinates of the ball's center
def ball(x, y):
    set_fill_color(1, 0.7, 0.2)
    disable_stroke()
    draw_circle(x, y, BSIZE)

# Purpose: determine if the right wall has been hit and game over if yes
# Parameter: right-most point of the ball
def rwall(ball_x):
    global play, vx, vy
...
# Purpose: determine if the left wall has been hit and game over if yes
# Parameter: left-most point of the ball
def lwall(ball_x):
    global play, vx, vy
    if ball_x <= BSIZE:
        vx = 0
...
# Purpose: determine if the top wall has been hit and bounce off if yes
# Parameter: top-most point of the ball
def twall(ball_y):
    global vy
    if ball_y <= BSIZE:
        vy = -vy

# Purpose: determine if the bottom wall has been hit and bounce off if yes
# Parameter: bottom-most point of the ball
def bwall(ball_y):
    global vy
...
# Game functions
# ----------------------------------------------------------------------------------------------------------------------
# Purpose: determine if the ball has hit a paddle and bounce off if yes
# Parameter: none
def paddle_touch():
...
# Purpose: starting screen of the gamed
# Parameter: none
def game_start():
    enable_stroke()
    set_stroke_color(1, 1, 1)
...
# Purpose: end game if "q" is pressed
# Parameter: none
def game_end():
    if qkey:
        cs1_quit()
